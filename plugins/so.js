...
`;
        const buttons = [
          {
            buttonId: `play_audio_${m.from}`,
            buttonText: { displayText: "Play Audio" },
            type: 1,
          },
          {
            buttonId: `play_document_${m.from}`,
            buttonText: { displayText: "Play Document" },
            type: 1,
          },
        ];

        playing[m.from] = filePath;
...
Matrix.ev.on("messages.upsert", async (update) => {
  ...
  if (update.messages[0].message?.buttonsResponseMessage) {
    ...
    const from = update.messages[0].key.remoteJid;
    if (buttonId.startsWith("play_audio_")) {
      const filePath = playing[from];
      if (!filePath) return;
      const doc = {
        audio: {
          url: filePath,
      ...
      await Matrix.sendMessage(from, doc, { quoted: update.messages[0] });
