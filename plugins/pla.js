...
// David Cyril API configuration
const DAVID_CYRIL_API_URL = 'https://apis.davidcyriltech.my.id/play';
...
// Utility function to fetch audio from David Cyril API
async function fetchAudioData(videoUrl) {
  const apiUrl = `${DAVID_CYRIL_API_URL}?query=${encodeURIComponent(videoUrl)}`;
  ...
  // Handle different response formats from David Cyril API
  let downloadUrl;
  ...
  try {
    ...
    if (command === "pla") {
      ...
      try {
        ...
        // Fetch audio data from David Cyril API
